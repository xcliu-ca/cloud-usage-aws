...
// slack related
const MENTIONS = {
  "ciddread@us.ibm.com": "icp-cicd",
  "c3cvt3vm@ca.ibm.com": "icp-sert"
}
...
vcore.watchDeep(aws_ec2, () => {
  ...
  setTimeout(() => {
    aws_ec2_active.value.forEach(instance => {
      ...
      if (tag_name !== -1) {
        console.log(instance.Tags[tag_name].Value)
      }
    })
  }, 100)
  setTimeout(() => {
    ...
    code.value = JSON.stringify(clusters, "", 2)
    slack.chat.postMessage({type: "mrkdwn", text: text.value, channel: channel.value}).then(() => flag_slack_working.value = true).catch(() => flag_slack_working.value = false)
  }, 200)
})
